...
|---|---|---|
| [`petri/`](petri/) | **L3a — weighted tactic-place Petri nets** | the five committed structural + weighted nets (four GASP classes + the aggregate null profile), the **synthetic overlay** ([`petri/synthetic_overlay.json`](petri/synthetic_overlay.json) — declared bidirectional pre-intrusion structure: forward chain `recon → resource-development → initial-access` + backward regression bridge `initial-access → reconnaissance`, composed at net construction, kept apart from the observed nets) and the **outcome (policy) overlay** ([`petri/outcome_overlay.json`](petri/outcome_overlay.json) — the success/failure conditional-likelihood weighting), the divergence-from-aggregate report, and the gitignored `_viz/` net diagrams. Generated README: [`petri/README.md`](petri/README.md). Build code: [`src/mtdsim/l3_simulation/petri/`](../../src/mtdsim/l3_simulation/petri). |
