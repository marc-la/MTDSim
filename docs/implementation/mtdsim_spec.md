...
| **ATK-05** | **Confusion / time penalty** on every MTD interruption; forced re-scan | Brown §V-A; Zhang §4.4.3 | Brown / Zhang | `attack_operation.py:153-182` `_handle_interrupt` + `ATTACK_DURATION['PENALTY'] = 20`; MTD triggers `attack_process.interrupt()` (`mtd_operation.py:219`, in `_interrupt_adversary` at `:211-239`). Locators refreshed 2026-07-27. | **verified on the native arm; divergent on the movement arm** (split, 2026-07-27). The native FSM catches the interrupt in `_execute_attack_action` and routes it through `_handle_interrupt`, paying the penalty: measured 96–124 interrupts consuming **1 964–2 536** t/u over a 15 ks `simultaneous` run (13–17 % of the horizon). The driven arm's `step()` has no handler — the interrupt propagates to `MovementAttacker._dispatch`, which reads it as a failure verdict and routes — so `_handle_interrupt` never runs and **0.0** penalty time is consumed, despite 74–108 recorded interrupt events. The re-raise is deliberate (running `_handle_interrupt` would re-impose native succession behind the driver); losing the penalty with it was not. **Stated comparability limitation, not fixed here** — the fix is S3's, which makes the penalty a movement-layer net place under the same stochastic regime. Cross-arm MTTC comparisons must state it. Pinned by `tests/test_action_layer_dispositions.py`. | Y | [CORE] |
