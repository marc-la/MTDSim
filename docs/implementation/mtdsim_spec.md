...
1. Each artefact has a stable **ID** (`NET-XX`, `ATK-XX`, `MTD-XX`, `SHD-XX`, `MET-XX`, `SIM-XX`, `CFG-XX`) and a **Disposition** against the running baseline:
   ...
   - `fixed` — was previously `divergent`/`missing`, repaired in a documented commit; one-line note records the spec-ID and commit context.
...
| **C3** | Service-compromise threshold & impact range | Brown Table I: impact `[0,1]`; README previously implied the same; threshold `7` makes sense only against `[0, 10]` | `services.py:26` `self.impact = random.random() * 10` → impact ∈ **[0, 10]**; `SERVICE_COMPROMISED_THRESHOLD = 7` against that range | **Resolved in 2c (docs).** Disposition: keep code `[0, 10]`; align README + spec NET-13 to `[0, 10]`; record the Brown-paper `[0, 1]` delta as inherited and not actioned (the threshold of 7 is sensible only against [0, 10], so flipping the impact range would also require recalibrating the threshold — out of scope for 2c). |
...
| **NET-13** | **Vuln impact ∈ [0, 10]** *(see C3)* | code (inherited); cf. Brown Table I `[0,1]` | inherited / disputed by Brown | `services.py:26` `self.impact = random.random() * 10` → **[0, 10]** | **fixed (docs, 2c)** — README + spec aligned to the code-side `[0, 10]`; Brown-paper `[0, 1]` delta documented but not actioned | Y | [CORE] |
| **NET-14** | **Service compromised when Σ exploited impact > 7** *(see C3)* | README; not in any paper at this granularity | inherited | `services.py:240` `return self.exploit_value > constants.SERVICE_COMPROMISED_THRESHOLD` with threshold = **7** | verified — threshold of 7 is calibrated against the `[0, 10]` impact range now documented in NET-13 | Y | [CORE] |
...
| **ATK-04** | **Adversary learning**: exploit time **halved** for previously-exploited vuln **types** (Zhang). Active code halves exploit time on previously-exploited vuln **instances** (Brown-era, distinct mechanism). | Zhang §4.4.3 | Zhang (per-type, missing); Brown 2021 (per-instance, active) | Two mechanisms distinguished: (a) `services.py:90-91` `if self.exploited: return exp_time / 2` is the **per-instance** discount, Brown-era (commit `a16db997`, 2021-09-04), **active** — fires in 7–42 % of `exploit_time` calls across the 9 goldens (pinned in `tests/test_atk04_reexploit_discount.py`). (b) Zhang's per-vuln-**type** halving has no active code; the commented-out `exploit_attempt + 1` line at `services.py:97-98` is its only remaining trace. | **divergent (split)** — (a) active per-instance discount kept deliberately (Unit C disposition); (b) per-type learning unimplemented. See metrics_semantics.md §c | Y | [ATK-SWAP] |
| **ATK-05** | **Confusion / time penalty** on every MTD interruption; forced re-scan | Brown §V-A; Zhang §4.4.3 | Brown / Zhang | `attack_operation.py:153-182` `_handle_interrupt` + `ATTACK_DURATION['PENALTY'] = 20`; MTD triggers `attack_process.interrupt()` (`mtd_operation.py:219`, in `_interrupt_adversary` at `:211-239`). Locators refreshed 2026-07-27. | **FIXED 2026-07-27** — was split (`verified` native / `divergent` movement). The native FSM catches the interrupt in `_execute_attack_action` and routes it through `_handle_interrupt`, paying the penalty: measured 96–124 interrupts consuming **1 964–2 536** t/u over a 15 ks `simultaneous` run (13–17 % of the horizon). The driven arm's `step()` has no handler — the interrupt propagates to `MovementAttacker._dispatch`, which reads it as a failure verdict and routes — so `_handle_interrupt` never runs and **0.0** penalty time is consumed, despite 74–108 recorded interrupt events. The re-raise is deliberate (running `_handle_interrupt` would re-impose native succession behind the driver); losing the penalty with it was not. Both arms now consume the substrate's `apply_mtd_interrupt_cost`, which carries the penalty **and** the lost host connection (B-INT-01) — the two consequences that are substrate facts. The driver still owns *succession* (no forced re-scan), which is the carve and is intended. S3 may later re-home the penalty onto the movement layer as a net place; the seam does not need to move for that. Pinned by `tests/test_action_layer_dispositions.py` and `tests/test_des_step.py`. | Y | [CORE] |
...
| **ATK-07** | **Give-up rule**: per-host attempt limit (= `ATTACKER_THRESHOLD` constant); **never** give up on the target node in Scenario 2 | Brown §III-C(2), Table I (=10); Brown §V-C ("never give up") | Brown | `constants.ATTACKER_THRESHOLD = 10`; `attack_operation.py:268-273` (`_do_enum_host`) increments `_attack_counter[curr_host_id]` and pushes the host into `stop_attack` **only when** `curr_host_id != target_node` **and** `network.network_type == 0`. **The push is gated on the targeted network, not exempted by it** — an earlier wording of this row ("push … unless `network_type==0 and …`") inverted the guard's polarity. | **FIXED 2026-07-27** (was `verified`, then re-dispositioned `divergent (unreachable)`, then repaired once the supervisor authorised bug fixes). The guard's polarity is restored to Brown's: give up after 10 attempts on any host, except the target node of a targeted network. Hosts are now bounded at 10 attempts and the give-up list survives `SCAN_NEIGHBOR`, which used to re-queue blacklisted hosts. Evidence for the divergence, retained: Three facts: (1) both arms build a `TimeNetwork` (`network_type == 1`, `get_target_node()` is `None`), so the rule never fires; (2) `network_type == 0` is unreachable **repo-wide** — `TargetNetwork` is never instantiated and its `copy_network` (the only setter of `0`) is never called, so the targeted scenario is dead code; (3) the inactivity **bites**: across no-MTD/simultaneous/random × seeds 0/42/1234, `stop_attack` is empty in every cell while hosts exceed Brown's bound of 10 — seed 42 no-MTD reaches a per-host counter of **50** (6 hosts over, 99 excess attempts). Recorded, **not fixed**: activating it re-baselines every golden (Marc's call). Supersedes the "re-run with the targeted scenario to disposition ATK-07" next-step for the *general* half; the targeted half stays unverified because it is unreachable. | Y | [CORE] |
| **ATK-08** | **Global attack-attempt cap**: `5 × total_nodes` | inherited (not from any paper) | inherited (Brown-era heuristic) | `Adversary.__init__`: `_max_attack_attempts = HACKER_ATTACK_ATTEMPT_MULTIPLER (=5) * total_nodes` (= 250 at the 50-node geometry). `_curr_attempts` is incremented in `_do_exploit_vuln` (`attack_operation.py:386`); the **single** enforcement site (`attack_operation.py:276-278`, in `_do_enum_host`) is **commented out**. Locators refreshed 2026-07-27 (they predated the carve). | **divergent (inert) — left inert deliberately**, confirmed 2026-07-27. The cap is genuinely overrun, not merely unenforced: the seeded no-MTD golden reaches `_curr_attempts = 481` against a cap of 250, and every sampled MTD cell exceeds it (474–713). Restoring the guard would truncate every run and break the 692/41 golden, i.e. a re-baseline (Marc's call). Combined with its paper-free origin — there is no source to be unfaithful to — the disposition is to keep it inert and documented rather than restore it. | Y | [PERIPH] |
...
| **SHD-05** | **Static-feature branch** (`Dense 128 + ReLU + BN + Dense 64 + ReLU + BN + Dropout 30 %`); inputs incl. **HCR, Number of Vulnerabilities, Number of Exposed Vulnerabilities, Attack Path Exposure Score** | Tay §4.1.1 | Tay | Architecture in `mtdai/mtd_ai.py` matches Tay's layer counts; feature list expects metrics defined in `statistic/security_metric_statistics.py` and `statistic/utils.py` | unverified | N | [TAY-BENCH] |
| **SHD-06** | **Time-series branch** (`LSTM 64 → BN → LSTM 32 → BN → Dropout 30 %`); inputs incl. **MTTC, Downtime/Operational Impact, Time Since Last MTD** | Tay §4.1.2 | Tay | Same as `SHD-05` — architecture present; not exercised in baseline | unverified | N | [TAY-BENCH] |
...
| **SHD-10** | **Static Degrade Factor (SDF) = 2000 ms** — if `now − last_MTD > SDF`, force a random MTD; check runs *before* metrics feed into the agent | Ho §3.1.2, Fig 2 | Ho | Look for `static_degrade_factor` / `SDF` in `mtdai/mtd_ai.py` and `mtd_ai_operation.py` — code path exists in `operation/mtd_ai_operation.py` | unverified | Y | [TAY-BENCH] |
...
| **SHD-13** | **Attacker-detection-rate (IDS sensitivity) experiment** — feed model varying fractions of attacker-action information (0–100 %); **performance cutoff ≈ 0.7** | Tay §5.3, §6.4, Fig 6 | Tay | Look for `attacker_sensitivity` / `detection_rate` params in `mtd_ai_operation.py` (`execute_ai_model` in the deleted driver had an `attacker_sensitivity` kwarg) | unverified (parameter present, behaviour not run) | Y | [TAY-BENCH] |
...
| **MET-08** | **Time Since Last MTD (TSLM)** = `now − last_MTD_executed_time` | Ho §3.3.2 (#9) | Ho | `time_network.py` carries `last_mtd_triggered_time` (init = 0); `evaluation.py` hard-codes `time_since_last_mtd = 1` and comments out `env.now - self._network.last_mtd_triggered_time`. **2c finding:** the field is only updated by the Tay RL paths (`mtd_ai_operation.py`, `mtd_ai_training.py`); the time-based scheduling path (`mtd_operation._mtd_execute_action`) never writes it. Uncommenting the env-dependent line would therefore return `env.now − 0 = env.now` for every time-based run — not "time since last MTD". A faithful fix needs both an MTD-side write on each trigger AND the in-evaluation call site (line 122) to pass `env`. No baseline consumer reads the TSLM cell from `state_array`, so the hard-coded `1` is currently inert. | **deferred** (cross-module wiring; no consumer in baseline; Tay path computes TSLM correctly inline) | Y | [PERIPH] |
| **MET-09** | **Shortest Attack Path Variability (SAPV)** = cardinality of set difference between consecutive shortest-path sets | Ho §3.3.2 (#10) | Ho | `evaluation.py:167-168` computes `shortest_path_variability` from `scorer.shortest_path_record` | verified (shape) | N | [PERIPH] |
| **MET-10** | **Network Address Variability (NAV)** = address changes / state size; ∈ [0, 1] | Ho §3.3.2 (#11) | Ho | Not visibly computed in `evaluation.py`. Likely lives in `statistic/security_metric_statistics.py` or `utils.py`. | unverified | N | [PERIPH] |
...
4. **ATK-04** — **split**. Zhang's per-vuln-**type** halving is **missing** from active code. A separate Brown-era per-**instance** discount (`services.py:91-92`, `if self.exploited: return exp_time / 2`) **is active** and fires in 7–42 % of `exploit_time` calls across the 9 goldens — a stronger divergence from a strict "no attacker learning" assumption than the pre-Unit-C wording implied. Kept deliberately and pinned by `tests/test_atk04_reexploit_discount.py`. See metrics_semantics.md §c.
...
| C3 | Impact range `[0, 1]` (Brown paper) vs `[0, 10]` (code) | **Resolved in 2c (docs):** keep code `[0, 10]`; README and spec NET-13 aligned to `[0, 10]`; Brown-paper `[0, 1]` delta noted, not actioned. |
...
- ~~Re-run Phase-0 with the **targeted attack scenario** (`network_type = 0`, target_layer set) to disposition NET-17 / ATK-07 / ATK-12 against real outputs.~~ **Blocked, and partly answered (2026-07-27).** The targeted scenario cannot be re-run as things stand: `network_type == 0` is unreachable repo-wide — `TargetNetwork` is never instantiated, and `copy_network`, the only code that sets `network_type = 0`, is never called. ATK-07's *general*-scenario half is now dispositioned against real outputs (`divergent (unreachable)`, see the row); its targeted half, and NET-17's, stay unverified **because the path is dead code**, not because the run is outstanding. Reaching them first requires constructing a targeted network — a scenario-design decision for Marc, not a spec follow-up.
