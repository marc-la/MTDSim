...
|---|---|---|
| **TOKEN** | `ENTER` (place + visit count) · `DWELL` (the sampled dwell **vs the catalogue mean**) · `STEP` (place → verb → verdict → next place, with the dwell/verb time split) · `END` (terminal tag) | The S3 stochastic-timing regime per event; the walk's shape; the per-step time decomposition; the terminal reason (objective / stall / sink / horizon / `MAX_EVENTS`) — never an unexplained stop |
...
**Extension log.** 2026-07-28: the unified L3 tracer (`mtdsim/l3_simulation/trace.py`) — TOKEN and CONTROLLER actors over the movement layer, collaborator-proxy instrumentation, the `run_movement` parity gate.
