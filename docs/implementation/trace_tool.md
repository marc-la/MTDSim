...
The substrate actors (ATTACKER, COMPROMISE, DEFENDER, MUTATION, INTERRUPT, NETWORK) carry over unchanged — the same hooks fire, so cross-layer events interleave. Two actors join them always, and a third only when a run attaches an attacker state:
...
| **STATE** *(only with `--demo-state` / `attacker_state=`)* | `OBSERVE` (place + verdict, with the state's running visit/verdict summary) · `MODULATE` (the destinations a modulator reweighted and by how much — emitted only when some factor ≠ 1.0) | The within-run attacker state accumulating knowledge, and the generalised-composition third factor actually conditioning routing (attacker-state seam). Silent on a stateless run and on a null-configured state, so its presence never implies an effect the run did not have |
...
The verdict section gains a **The attacker state** block when a state is attached, naming the modulators, what the state observed, and how many routing decisions were reweighted (0 for a null configuration — stated, so the null-equivalence is legible in the log).

**How it instruments.** The driver takes its collaborators by injection, so `phase_for` / `compose` / `verdict_of` / `timing.draw` are wrapped in recording proxies — no monkeypatching. The attacker state, when present, is wrapped the same way (`_TracedState`) and sits *inside* the stateful wrappers, so it observes and modulates exactly as the bare state would while the recording proxies outside it show the final routed distribution. Only two substrate seams are class-patched (with undo), because the driven arm bypasses the seams the substrate tracer hooks: `AttackOperation.step` (verbs run through the carve, not `_execute_*`) and `apply_mtd_interrupt_cost` (the driven arm's penalty path — it never calls `_handle_interrupt`). The per-step TOKEN summary is derived from the driver's own `MovementRecord` stream as appended, the one instrumentation point that cannot disagree with what the run records.
...
- 2026-07-28: the **STATE actor** — narrates the within-run attacker-state seam (`OBSERVE` / `MODULATE`) and adds the verdict's *The attacker state* block, behind `--demo-state` / `attacker_state=`. Silent on a stateless run; the `run_movement` parity gate now covers a stateful run too (`test_a_stateful_run_traces_identically_to_run_movement`). See [`pipeline/ogasp/attacker_state_seam.md`](pipeline/ogasp/attacker_state_seam.md).
...
- 2026-08-27: **measured write sets on every MUTATION line** (substrate tracer, inherited by the unified tracer). The `mtd_operation` hook now snapshots the network and the attacker's holdings either side of the firing (read-only, no RNG) and reports what actually moved: hosts swapped / IPs moved / edges rewired / OS relabels / services redrawn / ports moved / user sets redrawn, endpoint writes, the compromised set before→after with a `!!` if it was disturbed or the network/adversary lists disagree, the visible set before→after, and what the attacker had in hand (its target host swapped away; how many of its scanned ports went stale). The verdict gains a *What each mutation actually wrote (measured)* block, per mechanism. Built to verify the pool restoration (the retired pool-restoration brief (`git show d127f443:docs/handoffs/2026-08-27_mtd_pool_restoration.md`)): the canned `_describe` sentence stays as the headline, the numbers beside it are the evidence. Also: `--mtd` on the unified tracer (repeatable; `single` takes exactly one), mirroring the substrate tracer's selector.
- 2026-08-27: **`EXPLOIT REFUSED` attacker event + `exploits_refused_by_os` tally** (both tracers; `Vulnerability.network` wrapped read-only, re-reading the gate's own inputs, no draw). Narrates the OS-gated success channel reinstated under D-19 the same day. The MUTATION write line now prints services redrawn as a fraction of the internal pool, so OS diversity's selective redraw (D-18 repaired) is legible beside service diversity's full one.
- 2026-08-30: **the fresh-host contract and the token-hold rule on the STEP line** (unified tracer). Every action record now says when its verb fired on a host already owned (`[on a host ALREADY OWNED]` — the re-compromise churn, visible per event), when the fresh-host guard re-selected before firing and how many owned pops the retry-until-fresh loop took, and when the token was `HELD ×n` under register T1's rule with the extra dwell it paid. The verdict gains *The host-selection contract* (re-selects, re-pops, compromise verbs on owned hosts — 0 by construction with the contract on, the churn count with it off) and *The token hold (register T1)* (decisions consulted, held, abstained, fell through the bound, time spent holding). New flags: `--no-fresh-host` (the pre-2026-08-30 attacker — the "before" half of the T5 trace Jin asked for) and `--hold`. Built for `fsm_token_hold_findings.md`; the parity gate is unchanged and green.
