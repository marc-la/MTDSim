...
| IS-MTD-02 | **CONFORMS** (delta) | `portshuffle.py`: reassigns every service node's port on every **non-exposed host** (target node excepted). Brown says "all exposed services"; code exempts exposed *hosts* entirely (in-code justification comment) and shuffles all services on the rest. Latent (not in default set). *Activated 2026-08-27 as-is (Marc's ruling: faithful to Brown, limits are Brown's design); behaviour analysis in [`mtd_write_surfaces.md`](mtd_write_surfaces.md) §(c). No disposition opened — nothing diverges.* |
| IS-MTD-03 | **CONFORMS** | `usershuffle.py`: re-draws each host's users from the network user list. Its *blocking* behaviour diverges — see IS-INT-03. *(Annotated 2026-08-03, boundary review 2 cycle 2: the re-draw itself conforms, but `set_host_users` carries two documented-nowhere side effects that ratchet on every repeat call — `total_users` increments without reset (`host.py:490-491`, the D-26 divisor, so each firing hardens every host) and `p_u_compromise` latches True permanently (`host.py:492-494`). Latent (UserShuffle is outside the default set) → D-32.)* *Repaired 2026-08-27 (D-32 + D-26): counters recomputed per call, `total_users = len(users)`; exposed endpoints now exempt (R2, family rule D-23); pool `full`. Rulings R1/R2 in the retired pool-restoration brief (`git show d127f443:docs/handoffs/2026-08-27_mtd_pool_restoration.md`).* |
| IS-MTD-04 | **CONFORMS** | Same-layer host swap (`hosttopologyshuffle.py:29-55`), pairs drawn within `host_id_list_in_layer`; the same-layer constraint is enforced, exposed endpoints exempted (undocumented but consistent with layer 0 = endpoints). Latent (not in default set). Adversary id-keyed state remapped on swap (`adversary.py:28-60`). *(Annotated 2026-08-03, boundary review 2 cycle 2: the remap is incomplete on the network side — rebinding `_compromised_hosts` (`adversary.py:49`) severs the alias `update_reachable_compromise` establishes (`network.py:735`), so `network.compromised_hosts` keeps pre-swap ids and `hosttopologyshuffle.py:59` rebuilds `reachable` from them, erasing the attacker's foothold from the visibility model; the id-keyed scorer series and ip-feed are also never remapped. Latent → D-31.)* *Repaired 2026-08-27: in-place remap (alias preserved), all id-keyed adversary state moved, ip-feed refreshed; the historical `CompromiseStatistics` series is left as a time-stamped log by ruling; pool `full`. Regression test `test_hts_moves_the_foothold_with_its_instance`.* |
...
| IS-MTD-08 | **DIVERGES-DOCUMENTED-NOWHERE** (revised 2026-07-29, was CONFORMS-latent — see §m3; **ruled 2026-08-27: D-17 (c) — withdrawn from the pool and ruled out for this thesis**, see the D-17 ruling banner in §n) | `osdiversityassignment.py`: the MIP *scaffolding* matches Zhang's documented abstraction (single-source/single-destination reduction, endpoints + database as client classes), but the formulation is decoupled: the assignment binaries `s` appear in no objective term and in exactly one constraint (one-variant-per-node), because the `s`↔`f` coupling constraints — the docstring's own constraint 7 — are commented out (`osdiversityassignment.py:229-233`). CBC presolve reduces every instance to "0 rows, 0 columns — nothing to do"; the returned assignment is an arbitrary feasible point, so the mechanism does not solve the DAP it is documented to solve. Full evidence and disposition options in §m3 → D-17. Reuses OSDiversity's name for duration/priority lookup → inherits 80 s, matching Zhang's DAP_OSDiversity 80 s. |
...
it), and only Marc's disposition makes it fixable → **D-17** *(ruled 2026-08-27: (c) withdraw, see §n)*.
...
### Opened 2026-07-29 by the mechanism cost audit — ruled 2026-08-27

> **Ruled 2026-08-27 (Marc) — D-17: (c) withdraw.** OSDiversityAssignment
> (the DAP-optimised OS diversity, IS-MTD-08) is withdrawn from the pool and
> the idea is **ruled out for this thesis, not deferred** (Marc: "kill that
> idea, it's not gonna work, and document that"). The grounds are all already
> on the record: the formulation is decoupled (constraint 7 never written,
> CBC presolve deletes the model, the returned assignment is an arbitrary
> feasible point — §m3 and the row below; the further defects in
> [`mtd_write_surfaces.md`](mtd_write_surfaces.md) §(g)4: destination-removal
> off-by-one, saturated variant-compromise probabilities, infeasible-MIP crash
> path, OSD/OSDA suspension-dict name collision); the idea is **unpriced** in
> this simulator — OS choice affects no attacker outcome, the OS success gate
> being inherited commented-out (D-19) and OS Diversity being ServiceDiversity
> plus a dead relabel (D-18) *(both since repaired the same day; the withdrawal
> stands on the formulation and objective grounds)*, so even a correct DAP would optimise a dimension
> the model does not price; the DAP presupposes a server/database class and
> client→server connectivity, i.e. a targeted objective, and every recorded
> experiment runs the untargeted type-1 arm (`target_node` None); and the
> mechanism is the cost ceiling (~19.6× no-MTD,
> [`mtd_mechanism_costs.md`](mtd_mechanism_costs.md)). Code left in place
> (`osdiversityassignment.py`, header comment records the ruling), not in the
> default set; no goldens moved. The restored pool is seven — the lineage four
> plus HostTopologyShuffle, UserShuffle, PortShuffle
> (the pool-restoration brief, retired 2026-08-30 (`git show d127f443:docs/handoffs/2026-08-27_mtd_pool_restoration.md`)).

| # | Item | What the code does | What the papers say | Options, costed |
|---|---|---|---|---|
| **D-17** *(ruled 2026-08-27 → (c) withdraw; banner above)* | **IS-MTD-08** the DAP formulation is decoupled (§m3) | 128 s/run building + solving a MIP whose presolve deletes it; the returned OS assignment is CBC postsolve's arbitrary one-hot completion, deterministic per alive-node set | Zhang §4.3.1.5: solve the DAP — maximise expected client connectivity over OS-variant assignment, objective + constraints from Newell et al. | **(a) Repair** — write constraint 7 fresh (the commented lines were never functional); most faithful to the cited DAP; the real MIP is *slower still* (binaries stop being presolved away; 60 544 flow variables × 168 binaries), needs formulation work + re-baselining of every OSDA stream. Days of work, and the mechanism stays the grid's cost ceiling. **(b) Replace** — drop the solver and assign what today's path effectively assigns; ~1000× faster and honest about being a heuristic; **not automatically stream-identical** (the postsolve completion is arbitrary-but-deterministic; a replacement matches it only if CBC's completion rule is reverse-engineered and pinned, otherwise streams move and OSDA-dependent results re-baseline). **(c) Withdraw** from the pool and record why — zero work, already outside the default set; the demonstration-arms grid is the only published consumer. Ranked recommendation: **(c) ≥ (b) ≫ (a)** for the dissertation's purposes — (a) only if the DAP is to carry evidential weight, which nothing currently published needs. |
...
the cache works. *(Ruled 2026-08-27: (c) withdraw — the formulation question is closed, not answered.)*

### Opened 2026-08-02 by the OS/Service Diversity indistinguishability brief — ruled 2026-08-27

> **Superseded 2026-08-27 (ruled by Marc): the OS gate is reinstated and the compatibility guard repaired — D-18 (a) and D-19 reclassified as documented intent (Brown §III-B(6), "avoiding any OS-specific exploits"); OS Diversity now relabels the OS, redraws only the services made incompatible, and OS-dependent vulnerabilities fail on a mismatched host. Both arms, every golden re-baselined. Record: the retired pool-restoration brief (`git show d127f443:docs/handoffs/2026-08-27_mtd_pool_restoration.md`) §OS diversity.**
...
| **D-21** | **Movement-arm interrupt exposure as mapping policy** — the `DWELL` sentinel and the recon-verb immune share | A dwell-only place announces `DWELL` (`attacker.py:118-128`), which the gate reads as interruptible by network + application (live-verified, incl. "OSDiversity hit the attacker mid-DWELL" in trace); the app-immune share is the recon-verb clock time — 17–43 % by profile under `v2_partial`, so application-class reach is 57–83 % of the movement attacker's clock against ~99 % of the native attacker's (measured; interrupt yields 0.70 vs 0.99 corroborate) | The gate's exclusion set is documented intent (IS-INT-05); no paper models a non-dispatching dwell state, so the sentinel's exposure is a movement-layer design choice, previously recorded in code only | **(a) Keep** (recommended): same gate, same documented semantics, applied to a traversal that genuinely does reconnaissance; the DWELL exposure prevents a 28–56 %-of-clock hiding spot; the asymmetry is stated beside any cross-arm comparison. **(b) Make dwell-only places app-immune**: one-line controller-seam change, no goldens, but shields half the defence family from 28–56 % of the clock (yield ~0.70 → ~0.21–0.30) — a hard-to-defend flattering of the profiled attacker. **(c) Re-map tactics**: a mapping-version decision, owned by the mapping-sensitivity programme. |
| **D-22** | **Interrupt-counter asymmetry under overlapping mutations** (flag-grade) | A second interrupt mid-penalty is absorbed by the movement arm (counted by `add_total_attack_interrupted`, no second penalty) while the native arm is structurally immune (dead process — never counted): neither arm double-pays, but the counter's meaning drifts ≈ 0.3 counts/run at `simultaneous` (127.3 stats vs 127.0 record-level), zero at single-mechanism conditions | Papers are silent on overlapping mutations; the absorb-don't-stack rule and the native immune window are both recorded code design | **(a) Keep and document** (recommended; a line in `metrics_semantics.md` only if cross-arm interrupt counts are ever reported). **(b) Suppress the absorbed count**: substrate coordination change, moves `simultaneous` goldens; not worth it on current reporting. |
...
#### Cycle 2 (2026-08-03) — the fresh-eyes adversarial pass, run at Marc's direction

Part A's gate had passed with its adversarial-pass item satisfied only
same-session; Marc directed a genuine second cycle. Method and full findings
in [`mtd_write_surfaces.md`](mtd_write_surfaces.md) §(g): a blind red-team
enumeration (no access to audit records), an exhaustive object-graph diff,
and live OSD/OSDA revocation checks. The reported family's write sets
reproduced exactly; everything new sits in the latent pool or the metric
plumbing. The §5 confidence gate re-passes at ≥ 95 % with the fresh-pass
requirement met as written (§(g), final paragraph). Three additions:
...
### Opened 2026-08-03 (boundary review 1, cycle 2)

D-28 was **ruled a fix** by Marc (2026-08-03: faithful Complete Topology Shuffle
is a given) and has landed — guard in the shared verb core, 67 of 69 movement
goldens re-baselined, the nine native goldens bit-identical, SIM-05 re-verified,
regression test in `tests/test_enum_host_visibility.py`
(`baseline/CHANGELOG.md`, 2026-08-03). Its sibling below was found while
diagnosing the residual undefended rate, and belongs to the **same defect class**:
an invariant the native FSM enforced by *call order*, which the carve dropped when
it moved succession out. That class is now enumerated across all six verbs
([`attacker_read_surface.md`](attacker_read_surface.md) §(l6)) and has exactly two
members — this is the second.

| # | Item | What the code does | What the papers say | Options, costed |
|---|---|---|---|---|
| **D-33** | **SCAN_NEIGHBOR is dispatched from hosts the attacker has not compromised** | `assert_action_context` requires only that `curr_host` is not `None` (`attack_operation.py:668-672`), and `_do_scan_neighbors` then queues every neighbour of that host. The native FSM reaches SCAN_NEIGHBOR only from a compromise branch (`_execute_scan_port` on reuse, `_execute_exploit_vuln` on COMPROMISED, `_execute_brute_force` on success); the controller owns succession and may dispatch it any time. Measured: **166 of 345 calls (48 %) fire from an uncompromised host in the movement arm, against 0 of 71 natively.** `_do_scan_neighbors`' own docstring already names the precondition the code does not assert — *"semantically only meaningful on a just-compromised host"* | **IS-PRC-05**: "On successful compromise: assume C2 functionality — internal services and hosts connected to the compromised host become visible." Brown Fig 3 box 9 is "Scan for Neighbors to the **Recently Compromised** Host". This is a statement about what compromise *grants*, not about ordering — so a driving layer free to vary order is not thereby free of it (contrast IS-PRC-06, a sequencing statement the movement layer exists to vary; see §(l6) of the read-surface record for the semantic/procedural division) | **Ranking effect measured before asking** (10 seeds, movement arm, new comparative arm — not a re-run): gating it moves `simultaneous` from third to **first**, and raises compromise counts everywhere else (none 5.60 → 7.80, os_diversity 4.70 → 7.00, service_diversity 4.30 → 6.70) because a blocked dispatch **redirects** this attacker rather than stopping it. **(a) Assert compromise** in `assert_action_context` — same shape and location as D-28's fix, reaches both arms, no mapping change; but it is a far larger behavioural change (48 % of dispatches vs D-28's 22 %) and re-baselines the movement goldens again. **(b) Keep and document** — records that the profiled attacker holds an *unearned* reconnaissance capability the inherited one does not, which bears on any claim that the two attackers are compared on equal terms. **(c) Treat as a controller-mapping question** — restrict the mapping rather than the substrate; keeps the substrate untouched but puts the fix in the layer the criterion already calls a chosen input parameter. **No recommendation offered**: unlike D-28 this is not obviously a faithfulness repair, because the movement layer is *designed* to dispatch out of native order, and whether IS-PRC-05 is a capability statement or a sequencing one is the judgement the ruling turns on |

### Opened 2026-08-03 (boundary review 3, cycle 2 — the independent adversarial pass)

Found by the red-team pass the review's confidence gate requires (fresh eyes,
the six-channel inventory as exclusion set) and re-verified by the main
session; full evidence in
[`boundary_attacker_defender_channels.md`](boundary_attacker_defender_channels.md)
§(g). Both are flag-grade with keep recommendations.

> **Renumbered on merge, 2026-08-05.** Reviews 1 and 3 ran on separate branches
> and both allocated D-26/D-27 the same day, so the numbers collided when the
> branches were merged into `dev`. Reviews 1–2's allocations are canonical — they
> are the ones the retirement-era chain, the write/read-surface records and the
> handoffs already cite. This review's two rows are reconciled against them
> rather than re-allocated blindly:
>
> - **its D-26 (shared global RNG streams) is the same finding as D-29 above**,
>   reached independently by the red-team pass from the other side of the
>   boundary. It is folded into D-29 as corroboration rather than kept as a
>   second row; the evidence below is retained because it is the stronger of the
>   two — it quantifies per-mechanism draw volume and names the operative rule.
> - **its D-27 (HostTopologyShuffle writes attacker state) is a distinct
>   finding** and is renumbered **D-34**.

#### Folded into D-29 — the shared-RNG coupling, from the defender's side

| # | Item | What the code does | What the papers say | Options, costed |
|---|---|---|---|---|
| **D-29** *(corroborating evidence, boundary review 3 cycle 2)* | **Both sides draw from the shared global RNG streams** (covert defender↔attacker coupling, live in the default configuration) | `exponential_variates` uses numpy's global stream (no `random_state`); mechanisms draw heavily from global `random` (CTS's `gen_graph()` ≫ OSDiversity > IPShuffle) while the attacker draws exploit times, penalty draws and the host-stack ordering tie-break (`attack_operation.py:292`) from the same streams — so at fixed seed, which defence runs perturbs the attacker's subsequent draws, with per-mechanism perturbation volume. Unbiased across seeds; every recorded comparison is means-with-CIs (verified against `expo02_ashen_lynx/analyse.py`, which disclaims significance), so no recorded ranking is biased. The movement layer's own three streams are insulated; the substrate cores it calls are not | No paper specifies stream discipline; SIM-05 pins determinism per configuration, not cross-condition pairing | **(a) Keep and document** (recommended): the operative rule is that **no paired-by-seed cross-condition analysis may ever be run** without first re-visiting this row — pairing would read stream desynchronisation as treatment effect. **(b) Per-side streams**: a substrate-wide refactor that moves every golden for a property no recorded claim needs. |

#### D-34 — the latent seventh channel

| # | Item | What the code does | What the papers say | Options, costed |
|---|---|---|---|---|
| **D-34** | **`HostTopologyShuffle` writes attacker state directly** — the latent seventh channel, per-mechanism in a boundary otherwise priced per class | `hosttopologyshuffle.py:57` calls `adversary.swap_hosts_in_compromised_hosts` (`adversary.py:28-60`), remapping six id-keyed attacker fields. The only mechanism touching its `adversary` argument (all default-set mechanisms ignore it — enumerated). Coherence-preserving by intent: the host swap breaks the id↔instance binding attacker state is keyed on, and the write moves the attacker's knowledge with the ids. Unreachable in every recorded run (`mtd_scheme.py:24`, commented out) | No lineage paper describes a mechanism editing adversary state; Brown/Zhang express host-topology shuffling through network state + the interrupt classes | **(a) Keep, rule-before-promotion** (recommended — the D-07 shape): if `HostTopologyShuffle` is ever promoted into a reported family, this channel gets its own review first, since it gives one mechanism a direct per-mechanism write no classmate has. **(b) Re-express via network state**: redesign for a latent mechanism, no current consumer. |

### Opened 2026-08-05 (the disruption-wiring brief — does the class-level model *arrive*?)

Full evidence — the (mechanism x verb x driving mode) truth table executed
against the four real mechanism objects, the realised channel traffic measured
per mechanism for the first time (7 conditions x 6 seeds x 2 arms), and the net
of the seven arm asymmetries — lives in
[`disruption_wiring.md`](disruption_wiring.md). Nothing here re-opens D-20:
class-level pricing is the documented model per Marc's 2026-08-05 ruling, and
§(b) of that record **verifies** it rather than assuming it — the gate, the
penalty (bit-identical to four decimal places) and the cursor clear carry no
per-mechanism term. What these rows carry is the separate question of whether
the model reaches both driving arms alike. It does not, in a measured amount.

One asymmetry was repaired in-session and needs no ruling: **A6**, the movement
record's inability to name the interrupting mechanism. Schema-only, no behaviour
change, no golden moved, and it is the instrument every measurement above
depended on (0 of 1 061 interrupts unattributed after it). Two more (A2, A3) are
already-ruled documented model, and A4 is already-ruled mapping policy — with
much stronger evidence behind the ruling than it had, recorded in §(f) of the
brief's record rather than re-opened here.

| # | Item | What the code does | What the papers say | Options, costed |
|---|---|---|---|---|
| **D-35** *(mapping policy — statement wanted, not a repair)* | **EXPLOIT_VULN is uninterruptible in the movement arm**, so the diversity family loses 89-97 % of its exploit-blocking windows in the arm the headline runs on | With a supplied duration the whole attempt costs one up-front `timeout` and the vulnerability loop then runs with **zero yields** (`attack_operation.py:743-747`, `charge_time=False`); the native arm yields per vulnerability (`:466-467`), giving up to `SERVICE_TOP_X_VULNS_TO_RETURN` = 5 interrupt windows. Measured EXPLOIT_VULN interrupts per firing, native → movement: OSDiversity 0.82 → 0.03, ServiceDiversity 0.49 → 0.06, IPShuffle 0.86 → 0.07, CTS 0.27 → 0.08 | Not documented-nowhere: it is the declared consequence of **S3-R** (the movement layer supplies every unit of the attacker's time, so the substrate's per-vulnerability pricing is declined by design). No lineage paper specifies how many interrupt windows an exploit attempt offers — IS-INT-02/04 state only that the attack event fails | **(a) Record as a comparability boundary** (recommended): zero code, zero goldens; state in `metrics_semantics.md` that application-class disruption reaches the two arms unequally and that the diversity family's cross-arm comparison carries this factor. **(b) Yield per vulnerability under supplied pricing too** — divide the supplied duration across the vulnerability loop so the attempt offers the same number of windows in both arms. Restores cross-arm parity on exactly the channel that is these mechanisms' *entire* effect (D-18/D-19), but it is a substrate timing change that moves every movement golden (D-05 procedure) and it partly re-opens S3-R, which was ruled deliberately. **(c) Keep and say nothing** — not recommended; the magnitude is too large to leave implicit |
| **D-36** *(candidate bug — repair recommended)* | **A network-class mutation arriving during an application-class penalty loses its cursor clear** | `apply_mtd_interrupt_cost` decides what to clear from the MTD it was *called* with, and `MovementAttacker._pay_interrupt_cost` reads `_interrupted_mtd` before the penalty and nulls it after. Executed against the real methods: the arriving network-class mutation fires the gate, increments `add_total_attack_interrupted`, is recorded on the operation — and the cursor is **not** cleared (control, network penalty + network arrival: cleared). Frequency measured: **1.0 absorbed interrupt per run at `simultaneous`, 0.0 in every other condition** | **IS-INT-01** (Brown B-INT-01): a network-layer mutation means the connection to the host is gone. §c names "violates an invariant the papers state" as evidence for *bug*, and this does — the gate fires for exactly that mutation and the documented consequence does not follow. Distinct from D-22's absorb-don't-stack rule, which is about the *penalty* and is ruled | **(a) Re-read the class at the point of decision** (recommended): have the cost function consult the operation's current `_interrupted_mtd` when one arrived mid-penalty, so the strongest class present is the one charged. Small and local; touches both arms; moves `simultaneous` goldens only (the sole condition where overlap occurs), so a bounded D-05 re-baseline, plus inverting the pinning test at `tests/test_interrupt_channel_semantics.py`. **(b) Keep and document** — zero risk, and the measured exposure (1/run at one condition) is the argument for it; the cost is that the position-destroying family is silently under-credited exactly where mutation pressure is highest. **(c) Clear on the union of both classes** — strictly more disruption than either mutation alone licenses; not recommended |
| **D-37** *(record-grade)* | **The confusion penalty is charged to no `attack_record` row**, on either arm | 8.0-17.7 % of the simulated clock is spent inside `apply_mtd_interrupt_cost` (measured: 397-879 t.u. per run; 17.7 % at `simultaneous`) and appears in no record row, so it is invisible to every record-derived metric. Asymmetric in *recoverability* rather than in cost: the movement arm can reconstruct it (`measures.mtd_penalty` derives `end - start - dwell`, which is how the figures above were taken), the native arm cannot | No lineage paper specifies the record schema; the penalty itself is faithful (Brown §V-A, Zhang §4.4.3) | **(a) Record-only** (recommended): a line in `metrics_semantics.md` stating that record-derived time budgets exclude the penalty on both arms and that only the movement arm can recover it. Zero code. **(b) Give the penalty its own row** on both arms — makes the two arms' time budgets directly comparable and would have to precede any cross-arm MTTC-shaped claim, but it changes the shape of `attack_record` and moves every golden that reads row counts |
| **D-38** *(record-grade — and explicitly **not** an arm asymmetry)* | **The priority queue gives the higher-priority member of each class pair a systematic extra firing** | Both class pairs share a capacity-1 SimPy resource and `env.process(...)` defers the request, so the occupancy check never sees the just-spawned sibling and contention resolves FIFO in priority order. Measured **identically in both arms**: `simultaneous` CTS 13.0 · IPShuffle 12.0 · OSDiversity 13.0 · ServiceDiversity 12.0 (native and movement alike, 6 seeds); `alternative` 7/6/6/6 in both. Priorities: CTS 1, IPShuffle 3, OSDiversity 4, ServiceDiversity 6 | IS-SCH-05/06 document suspend-or-discard and priority ordering; nothing documents a systematic per-firing advantage arising from the deferred request | **Because it is arm-invariant it cannot explain the ranking inversion and does not compromise the two-arm comparison** — which is why this is record-grade rather than a defect. What it does do is **spuriously separate each class pair in favour of its higher-priority member**, running counter to the unseparated-pair finding. **(a) Record and report per-firing** (recommended): state it wherever a within-pair difference is reported, and normalise per firing as `disruption_wiring.md` §(d) does. **(b) Check occupancy before spawning** — removes the advantage, moves every multi-mechanism golden, and buys nothing any recorded claim needs |
