...
| **L3 success/failure outcome overlay** — the M2 policy layer: each directed tactic-pair carries two declared multipliers (`success`, `failure`) in [0,1], composed multiplicatively with the D3 base weights and the substrate's binary verdict at runtime, and renormalised within the source place's surviving out-set. Applied **only at the six action-bearing places** (dwell-only places route on base weights unconditioned). Values default from a kill-chain band relationship rule (forward/lateral/backward); five AAR-grounded overrides deviate the `success` side; the `failure` side carries **no** override | Supervisor decision M2/M3 ([`pipeline/ogasp/supervisor_decision_register.md`](pipeline/ogasp/supervisor_decision_register.md) §M2/§M3); design + composition/stall/interrupt/lifecycle in [`pipeline/ogasp/success_failure_overlay_design.md`](pipeline/ogasp/success_failure_overlay_design.md); verdict oracle [`pipeline/ogasp/tactic_action_map.md`](pipeline/ogasp/tactic_action_map.md) §4; success-pattern grounding [`../sources/tactic_profiles/step_c/`](../sources/tactic_profiles/step_c/) (Sophos/DFIR AARs) | artefact [`data/ogasp/petri/outcome_overlay.json`](../../data/ogasp/petri/outcome_overlay.json); composition/stepping code is the profiled-attacker build (not yet written) | **editorial/synthetic (declared)** — a declared **policy** layer, not flow-derived and not reverse-engineered from the nets; conditions the grounded D3 base weights, never re-derives them; envelope-not-actor. The **evidence-tier asymmetry is load-bearing**: `success` is base-grounded (D3, from success-biased CTI) with AAR-grounded overrides, `failure` is a declared judgement prior (no report documents failure behaviour). **Awaits Marc's review.** |
