...
| **L3 outcome (policy) overlay** — the M2 policy layer: a **ground-up conditional-likelihood weighting over the whole directed tactic-pair set**. For each pair `a→b` and verdict `v∈{success,failure}`, a declared value in [0,1] = "given the action at `a` came back `v`, the likelihood of `b` next" (0 = won't happen, 1 = most likely next). Composed multiplicatively with the D3 base weights + the substrate's binary verdict at runtime, renormalised per source place. Whole-space coverage (all sources incl resource-development, forward + backward); runtime consumption tracks verdict availability (the action set is extensible). Values reasoned per-pair from kill-chain band + capability enablement (`enables`) + foothold dependency | Supervisor decision M2/M3 ([`pipeline/ogasp/supervisor_decision_register.md`](pipeline/ogasp/supervisor_decision_register.md) §M2/§M3); design in [`pipeline/ogasp/success_failure_overlay_design.md`](pipeline/ogasp/success_failure_overlay_design.md); verdict oracle [`pipeline/ogasp/tactic_action_map.md`](pipeline/ogasp/tactic_action_map.md) §4; success-pattern grounding [`../sources/tactic_profiles/step_c/`](../sources/tactic_profiles/step_c/) (Sophos/DFIR AARs) | artefact [`data/ogasp/petri/outcome_overlay.json`](../../data/ogasp/petri/outcome_overlay.json) (123-pair resolved table + model); composition/stepping code is the profiled-attacker build (not yet written) | **editorial/synthetic (declared)** — a declared **policy** layer, not flow-derived and not reverse-engineered from the nets; conditions the grounded D3 base weights, never re-derives them; envelope-not-actor. The **evidence-tier asymmetry is load-bearing**: `success` grounded in MITRE tactic semantics + AAR-attested enablement, `failure` is declared judgement (no report documents failure behaviour). **Awaits Marc's review.** |
| **L3 per-tactic state-duration regime** — tiered sourcing: (1) substrate verbs (`ATTACK_DURATION`) where a tactic maps onto them, (2) literature timing values where they exist, (3) reasonable **justified estimate** otherwise (e.g. stealth); every value carries provenance + a sweep range | Supervisor decision D4 (same session; no ready-made MITRE-tactic→time resource exists — defined here, with justifications); method + tier bar in [`../notes/2026-07-04_operational_validation_the_bar.md`](../notes/ch3_design/operational_validation.md); evidence layer in [`../tactic_profiles/`](../notes/ch3_design/tactic_profiles/) §5 | [`data/ogasp/tactic_durations.json`](../../data/ogasp/tactic_durations.json) (v0 uncalibrated); gate in [`tests/l3_simulation/test_durations.py`](../../tests/l3_simulation/test_durations.py) (key set = place-union; corpus-frequency guard; anchor arithmetic; magnitude sanity) | **dispositioned, built (v0 uncalibrated)** — plain per-state dwell consumed by the standalone runner, *not* a stochastic firing rate (timed-net semantics deferred per D10); tuned anchors calibrate within their sweep ranges, then freeze v1 — **calibration re-sequenced post-MVP** per the 2026-07-10 written feedback (register R1: no timing literature expected, justify qualitatively against practical reports — observations long, execution quick; R4: simulation settings are free experimental variables). The v0 declared values carry the pipeline MVP as-is. **Per-tactic rows below await Marc's approval.** |
