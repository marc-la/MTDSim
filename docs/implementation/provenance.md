...
| **M6 prefix-join declared weight** = 1.0 — the synthetic `reconnaissance → initial-access` transition added where the observed corpus leaves recon an island (`double_extortion`, `infrastructure_setup`) | Supervisor decision M6 ([`pipeline/ogasp/supervisor_decision_register.md`](pipeline/ogasp/supervisor_decision_register.md) §M6); shape + resolved open judgement in [`pipeline/ogasp/tactic_action_map.md`](pipeline/ogasp/tactic_action_map.md) §6 | [`src/mtdsim/l3_simulation/petri/prefix_join.py`](../../src/mtdsim/l3_simulation/petri/prefix_join.py): `DECLARED_WEIGHT`, `curate_prefix_join`; artefact [`data/ogasp/petri/prefix_join_overlay.json`](../../data/ogasp/petri/prefix_join_overlay.json); gate in [`tests/l3_simulation/test_petri.py`](../../tests/l3_simulation/test_petri.py) §7 | **editorial/synthetic (declared)** — no flow backing, flagged `synthetic`, kept outside the observed nets as a composed overlay; deterministic (sole out-transition of its source place, guard rule forbids mixing with observed out-edges, so the D3 flow-proportion layer is never renormalised). `resource-development` deliberately not bridged (documented island) |
