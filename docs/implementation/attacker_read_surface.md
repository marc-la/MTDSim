...
| **`host.os_type`** | **success path: never** — the gate is inherited commented-out code (`services.py:146-148`, D-19). Only live read is the ×2.5 mismatch multiplier (`services.py:116-117`, IS-TIM-06 beyond-paper) | **time** | **declined** — 159 reads native, **0 driven** | OSDiversity | **DEAD to outcome; time-only, native-arm only** |
