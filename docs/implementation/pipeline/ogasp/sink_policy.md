---
status: superseded — the implementation described here did not land; see sink_retrace_design.md
created: 2026-07-29
updated: 2026-08-01
...
one-shot edge suppression), on the stranded-axis-work handoff's ruling — it is the
version the axis-1/3/4 work builds on, and the suppression is a strictly stronger
guarantee costing one declared rule. This record is kept because its §3
out-degree inventory is the empirical argument that the suppression is
belt-and-braces rather than load-bearing (lifted into `sink_retrace_design.md`
§3.1), and because [`experiment_02_findings.md`](experiment_02_findings.md)'s
recorded runs were taken under **this** record's implementation — the two differ
only in the suppression, but that difference is behavioural (see the
reconciliation note in the findings record, §2).

**Original status:** durable design-and-build record. It discharges **Part A** of the
