...
| `profile_divergence` (via `jsd`, `visit_distribution`, `terminal_place_distribution`); null band via `split_half_divergence_null` / `divergence_report` | 2 (stronger claim) + 3 — "mirroring the L2 corpus-level check at the execution level" | pairwise Jensen–Shannon divergence between profiles' pooled visit streams and terminal-tactic distributions, in the L2 convention (`jensenshannon(p,q,base=2)**2`, union support); the execution-level null is the within-profile split-half JSD over seeded draws (2026-08-09, closing the blind spot this row carried) | the terminal half is noise-dominated at 50 runs/profile (null ceilings 0.27–0.40); and the **divergence-to-`aggregate` column is size-confounded** — the pre-registered kill criterion fired at Spearman ρ = −1.0 against flow count, so that column is never read as objective conditioning ([`profile_divergence_findings.md`](profile_divergence_findings.md) §3) |
