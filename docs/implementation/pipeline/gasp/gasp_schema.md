...
- **Structural-terminal is the dropped P1 candidate, not a fallback.** P1
  ...
  with the audit on 15 of 38 flows (40 %) under the any-overlap rule, 19 of 38
  under exact match — see
  [`../notes/2026-05-28_l2_partition_decision.md`](partition_decision.md) and,
  for the pinned definition and both rules,
...
| `terminal_techniques`, `terminal_tactics` | Structural metadata (descriptive — *not* the class mechanism, per Decision 3). Since 2026-08-17 generated by `tools/gasp_structural_baseline.py --write-descriptive` under the L1 contraction (Def A; [`structural_baseline.md`](structural_baseline.md) §(a)) |
| `reaches_exfiltration`, `reaches_impact` | Structural reach (descriptive; same tool) |
...
which disagrees with the audit on 15 of 38 flows (any-overlap; 19 of 38 exact
— [`structural_baseline.md`](structural_baseline.md) §(b)). The audit's own
descriptive columns make the point — only 7 of 19 `objective_exfiltration`
flows terminate on exfiltration, and none of the 5 `objective_none_c2` flows
terminates on command-and-control (two of them are cyclic and have no
terminal at all).
...
1. **ToolShell flow-split — withdrawn by evidence (2026-08-17).** The
   round-1 concern was that `toolshell_vulnerability_in_sharepoint` conflates
   two threat-actor clusters (CL-CRI-1040 → MachineKey exfil; 4L4MD4R →
   ransomware). Re-reading the flow file: it contains **no** ransomware /
   impact action — the analyst drew MachineKey extraction → exfiltration (×3)
   → ViewState persistence only; the 4L4MD4R half exists in vendor coverage,
   not in the flow. There is nothing to split; the flow is
   `objective_exfiltration` at high confidence
   ([`structural_baseline.md`](structural_baseline.md) §(d)). Left numbered so
