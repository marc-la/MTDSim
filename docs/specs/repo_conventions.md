...
- The Tay RL / benchmark path (`mtdnetwork/mtdai/`, `operation/mtd_ai_*.py`) runs under the current env (TF 2.21 / Keras 3.14; Tay's recovered pretrained weights load; mtd_ai smoke run completes). R1/R3 — which the AI surface had inherited — are fixed as of Phase 2b. Reuse-vs-retrain and the known feature-shape mismatch are dispositioned in [`architecture.md`](architecture.md#L70) §(a) Tay decision block.
