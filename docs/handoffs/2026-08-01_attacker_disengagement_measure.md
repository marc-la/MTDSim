---
status: open — the build has landed; read the merge banner below before starting
created: 2026-08-01
updated: 2026-08-05
---
...
> ## Merge banner, 2026-08-05 — most of this brief has already shipped, on a branch this one could not see
>
> The session branches were merged into `dev` on 2026-08-05. This brief was
> rescoped that morning on the boundary branch; the work it prescribes had by
> then already been **built and run** on `dev`, which deleted the handoff in the
> commit that shipped it (`35f772e`) exactly as the lifecycle requires. The two
> branches simply never saw each other. The brief is restored rather than left
> deleted because its **2026-08-05 rescope is not in the shipped record** — the
> promotion to *axis 6's metric* and the absorption of the iterated cost model
> are framing decisions the record does not carry.
>
> **Start from the artefacts, not from §3 onward.** The measure exists:
> [`../implementation/pipeline/ogasp/attacker_disengagement.md`](../implementation/pipeline/ogasp/attacker_disengagement.md)
> (480 runs, C2 moved at 2 of 5 profiles so the stopping rule fired) and its
> pre-registration
> [`attacker_disengagement_prereg.md`](../implementation/pipeline/ogasp/attacker_disengagement_prereg.md).
> The iterated cost model this brief declares retired was *also* built and swept
> on `dev` ([`iterated_cost_model.md`](../implementation/pipeline/ogasp/iterated_cost_model.md)),
> so "measurement comes first" was answered by running both.
>
