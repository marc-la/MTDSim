...
| **8** Scheme awareness | NOT ADDRESSED → under review | per primitive: repeat-compromise rate on previously-seen configurations vs unseen; correlation between target selection and mutation frequency; any invariant-feature channel | **now live** — the exclusion is under reversal for a proof of concept ([`2026-08-06_knowledge_gated_apt_attacker.md`](2026-08-06_knowledge_gated_apt_attacker.md), which absorbed the earlier brief) | `repeat_configuration_compromise_rate` — **largely discharged, and negatively.** That handoff's §3 measured it on 2026-08-06: **exact whole-host recurrence is zero** on any mutated host (the apparent 9.87 % is precisely the five never-mutated exposed endpoints), so primitive (i) in its exact-image form has nothing to find here. What survives is the *coarse* form — vulnerability-identity recurrence, which is large (two-thirds of live ids sit on more than one host). If this reader is still built, build it at that granularity and report the exact-image zero as the result it is |
