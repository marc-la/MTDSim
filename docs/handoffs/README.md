...
| # | What | Blocks |
|---|---|---|
| **Overlay: failure-only?** | Collapse the outcome overlay to a failure-only matrix (success = pass-through on the corpus proportions)? Feasibility shipped 2026-08-19 — [`success_null_overlay_feasibility.md`](../implementation/pipeline/ogasp/success_null_overlay_feasibility.md) §8: recommendation **keep `v3`, carry the study as the ablation, write §4.2.4's failure paragraph on the premise**; adoption as `v4` re-keys ≈ 27 000 published rows (§6). The kernel-only third arm is ruled out on expressiveness (§5.8). The sibling kernel-discrepancy ruling is open in [`failure_weight_decomposition.md`](../implementation/pipeline/ogasp/failure_weight_decomposition.md) §4 | the owed §4.2.4 failure-encoding paragraph |
