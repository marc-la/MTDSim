...
| **MET-08** | **Time Since Last MTD (TSLM)** = `now − last_MTD_executed_time` | Ho §3.3.2 (#9) | Ho | `time_network.py` carries `last_mtd_triggered_time`; computed in `evaluation.py:188-189` but with `time_since_last_mtd = 1` hard-coded (commented-out env-dependent calc above it) | divergent (hard-coded to 1) | Y | [PERIPH] |
