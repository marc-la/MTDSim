...
**Maps to:** [`../specs/architecture.md`](../../implementation/architecture.md) §(j) (methodological positioning — the published precedent the §(j) four-axes contrast is drawn against); [`../specs/architecture.md`](../../implementation/architecture.md) §(e) L2 GASP (the *input class* and *temporal stance* axes are the structural difference).
...
**Maps to:** [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md) §III-D line 137 ("the manual cost shifts from narrative curation to authoring and maintaining the technique-labelling rules") — this is the lit-review claim Rodríguez most directly anchors.
...
**Maps to:** [`../specs/architecture.md`](../../implementation/architecture.md) §(j) — specifically the *output use* axis (Rodríguez: process model for forensic interpretation; this thesis: sampling-ready behavioural profile driving MTD evaluation); also §(l) where the open question of whether GASP should be encoded as a Petri net is noted as a *separate workstream*, not a Rodríguez-style methodology.
...
- [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md):26 (§I introduction) — used to anchor the umbrella claim that attack profiling reconstructs adversary behaviour as structured TTP-level representations.
- [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md):97 (§III-A MITRE ATT&CK) — used as the source of the tactic/technique/sub-technique/procedure hierarchy definition (see source line 73 of the paper, §2.2).
- [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md):131 (§III-D Attack Profiling, opening sentence) — anchors the attacker-trait framing; see Concept 2 for the paraphrase-mismatch flag.
- [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md):137 (§III-D, PM family) — names Rodríguez as the PM-based extraction exemplar, "discover[ing] attacker process models from labelled runtime event logs, recovering process structure in formalisms such as Petri nets"; see Concept 3 + Concept 4.
- [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md):215 (§V Synthesis) — grouped citation `[7], [16], [17]` for the three automated-extraction exemplars (PM / NLP / LLM), supporting the claim that the means to close the procedural-semantic gap already exist in MTD-adjacent literatures.
