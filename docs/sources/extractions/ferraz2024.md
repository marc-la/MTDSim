...
**Maps to:** [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md) §III-A (line 103) and §III-D (line 131) — both passages cite [13] for this exact construction; [`../specs/architecture.md`](../../implementation/architecture.md) §(c) L0 framing, where Attack Flow is brought in precisely to supply the dependency/ordering structure that Ferraz documents as absent from raw STIX.
...
**Maps to:** [`../specs/architecture.md`](../../implementation/architecture.md) §(j) methodological positioning — the boundary framing is the cleanest way to characterise what the L0→L2 build pipeline *does not* claim to supply (and where analyst curation enters); also relevant to [`../specs/architecture.md`](../../implementation/architecture.md) §(g) L4 validation framing.
...
**Maps to:** [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md) §III-A (line 101, "richest campaign-level structure" claim) and §III-D (line 131, the procedural-semantic-gap claim) both lean on this measurement; the result also underwrites this dissertation's choice in [`../specs/architecture.md`](../../implementation/architecture.md) §(c) to use the analyst-curated Attack Flow corpus as L0 input rather than attempting to derive structure from raw STIX campaign objects.
...
The dissertation makes a different substrate choice: the equivalent of Stage 3 lives inside MTDSim (a discrete-event simulator, not an emulator), and the operationalisation is graph-traversal inside the simulator rather than command execution against containers. This is documented as an explicit architectural decision in [`../specs/architecture.md`](../../implementation/architecture.md) §(a) ("Decision — no MITRE Caldera adversary emulation"; rationale: emulation overhead misaligned with simulation-based scope). The two substrates target different fidelity rungs: Caldera operates at executable-procedure fidelity (concrete commands against concrete hosts), MTDSim at simulated-traversal fidelity (technique-level events with timing). The framing layers above the substrate (procedural-semantics gap, sufficiency boundary, three-stage staging) carry across; the substrate layer does not.

**Maps to:** [`../specs/architecture.md`](../../implementation/architecture.md) §(a) "Decision — no MITRE Caldera adversary emulation"; §(f) L3 (the OGASP operationalisation layer); §(j) methodological positioning.
...
Cited as `[13]` in [`../sources/LIT_REVIEW.md`](../lit_review/LIT_REVIEW.md) (References list at line 261). In-text locators:
