...
| **ch5 results** — sensitivity, MTD evaluation | `ch6_results/defence_ranking_inversion` | `comparability_and_census` | `pipeline/ogasp/` experiment findings, the sweep records, `data/ogasp/` |
| **ch6 discussion** — interpretation, limitations | `ch7_discussion/` (all) | `movement_objectives`, `comparability_and_census`, `axis8_rise_and_fall` | `apt_model_criterion`, the measured-negative records |
| **ch7 future work** | `ch8_future_work/successor_programme` | `axis8_rise_and_fall`, `movement_objectives`, `learning_capability`, `incentive_rationality` | `apt_model_criterion` (reopening conditions) |
