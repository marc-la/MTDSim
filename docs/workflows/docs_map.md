...
The subdirs track the **ratified chapter structure** (supervisor register V-series, 2026-08-11; remapped 2026-08-14): introduction (ch1), background (ch2), literature review (ch3), APT attacker model (ch4), experimental setup (ch5), results (ch6), discussion (ch7), future work (ch8), conclusion (ch9). Restructured 2026-09-04 (Marc's ruling, trail in [`../implementation/research_record/threads/rq_and_structure.md`](../implementation/research_record/threads/rq_and_structure.md)): the methodology chapter's problem-definition section was cut as a duplicate of ch3 §3.3, the movement-attacker section became the chapter, and the experimental setup became its own chapter.
...
| `ch4_methods/` | APT attacker model (ch4) | the modelling arguments (corpus → profiles → attacker; validity defences) and the realisation arguments (built-beside, portability contract, bug-vs-design verification). Includes [`tactic_profiles/`](../notes/ch4_methods/tactic_profiles/) (the 15 per-tactic evidence profiles + their `_rubric.md`) |
