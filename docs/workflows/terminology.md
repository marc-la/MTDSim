...
status: living
created: 2026-08-20
updated: 2026-09-07   # RATIFIED row for the three members of the Cho Sec. VI triad (game-theoretic / genetic-algorithm / machine-learning; the ported "formulations" retired) --- proposed and ruled the same day at the ch3 §3.2.1 voice pass (Marc's blanket accept). 2026-09-05: attacker-model/threat-model split RATIFIED (a) thesis-wide (Marc, ch3 §3.2/§3.3 restructure); NEW ratified row "defence modelling approach" (Cho Sec. VI; the ch3 §3.2.1 heading). 2026-09-04: PROPOSED SQ-identifier row (ch1 list + \ref convention); genre-identification row AMENDED (opener noun = APT attacker model, ch4 pass 6); two PROPOSED rows from the ch4-preamble pass-6 split-stream census (L0 input cluster; action set vs action layer/verb); earlier same day: PROPOSED attacker-model/threat-model split row (§3.2.1 walk); 2026-09-03 flow-instance row (flow lowercase, Attack Flow reserved for the language) ratified from the §3.1.3 pass-6 split-stream census
---
...
| The genre identification | **movement attacker** is this work's name for the model ("we are putting names on things") — the identification is stated **once**, in the ch4 preamble opener, and after it *movement attacker* carries. **AMENDED 2026-09-04 (Marc, ch4-preamble pass 6):** the opener's genre noun is now **APT attacker model**, paralleling the chapter title ("The APT attacker model that we have produced, which we are terming the movement attacker") — his terminology-standardisation ruling; *threat model* no longer appears in the opener and recurs only when speaking the genre's language against the literature, never as a drifting synonym for our model | bare *threat model* as a name for our model anywhere — watch (census 2026-09-04: 0 in ch4 prose after the amendment) | Marc, 2026-08-20; amended 2026-09-04 |
...
| The modelled adversary, thesis-wide (own voice) | **attacker model** everywhere the thesis speaks in its own voice, including where it scores a surveyed work's model (ch3 §3.3.2 heading, Table 3.2 caption and cells, the cross-section prose, the ch2 baseline opener). Option (a) of the 2026-09-04 proposal, ruled 2026-09-05 during the ch3 §3.2/§3.3 restructure ("standardise terms"). The 2026-08-20 genre-language licence is NARROWED to quotations and cited section titles only | *threat model* — survives only inside quotations (hong2018 :73 in G1a) and cited titles; *attack model* (Cho's §V-D term) — never. Census 2026-09-05 after the sweep: *threat model* 0 in own-voice prose (ch2 l.584, the §3.2 opener, and nine §3.3 sites swapped); *attacker model* 20 | Marc, 2026-09-05 |
| The theory the DEFENCE's orchestration is designed under (Cho 2020 Sec. VI: game-theoretic, evolutionary/genetic, learning-based) | **defence modelling approach** (heading form *Defence modelling approaches*, ch3 §3.2.1). The qualifier is load-bearing: bare *modelling* is the attacker's word in this thesis (ch4), and an unqualified "Modelling approaches" beside "Attacker models in MTD" invites the misreading that it is our modelling. Names where the FIELD's attacker comes from; never used of this thesis's own method (which is an *evaluation method*, simulation over a graphical security model, Cho Sec. VIII) | *modelling tradition* / *attacker modelling tradition* (the retired §3.3.1 heading), *mode of modelling* (the §3.2.2 [3b], resolved), *theoretical approach* (Cho's own phrase — inside cited phrasing only), *modelling technique* (the ported P1's word — [term]-flagged for pass 6, not yet changed) | Marc, 2026-09-05 (ch3 §3.2 restructure) |
| The three members of the Cho Sec. VI triad (the defence modelling approaches) | **game-theoretic** / **genetic-algorithm** / **machine-learning** approaches --- adjectival, hyphenated as modifiers, Cho's own nouns (his section titles); the noun heads **game theory** / **genetic algorithms** / **machine learning** licensed as prepositional openers ("In game theory ...") | *formulation(s)* as a synonym for *approach* (the ported review's clause, swapped 2026-09-07); the ported triad *evolutionary / learning-based* --- never (paraphrases of Cho); unhyphenated *machine learning approaches*, *reinforcement learning agents* as modifiers --- hyphenate | Marc, 2026-09-07 (§3.2.1 voice-pass blanket accept; census at ratification: *game-theoretic* 4, *game theory* 2, *formulation* 0, *machine-learning* 4) |
| The way an MTD evaluation is carried out (Cho 2020 Sec. VIII's four categories: analytical, simulation, emulation, real testbed) | **evaluation method** (heading form *Evaluation methods*, ch3 §3.2.3; the strand opener's noun). Marc's ruling: *approach* is how the defence mechanism is designed (the row above); a method is not | *validation method* (the review's word; the ported §3.2.3 sentence was the last prose occurrence, swapped 2026-09-07); *approach* for any of the four categories ("the dominant approach" -> "the dominant method", §3.2.3 S4) | Marc, 2026-09-07 (§3.2.3 3b walk) |

### Proposed — awaiting Marc's ruling pass

| Concept | Recommendation | Alternatives considered | Census |
|---|---|---|---|
| The L0 input (the intelligence the pipeline starts from) | Two objects, not one: **cyber threat intelligence (CTI)** for the class (expanded once, ch3 §3.1.3, l.675), **the campaign corpus** for the instance mined (the Attack Flow corpus). *Campaign intelligence* is the drift item: ratify as the plain-English gloss of CTI at first mention (ch1) or retire it. Raised by the ch4-preamble pass-6 black box (2026-09-04) | (a) as recommended; (b) *campaign intelligence* everywhere in own voice, *CTI* only as the field's acronym; (c) one term, *CTI*, with *the corpus* as the instance | tex 2026-09-04 prose: *cyber threat intelligence* 2 (l.675 expansion; the §4.1 heading), *campaign intelligence* 2 (the ch1 sub-question sentence; the fig:pipeline caption), *campaign corpus* 0 in committed prose (1 in the proposed ch4 preamble) |
| What the movement attacker inherits from the baseline attacker | **action set** only if it names the six verbs collectively (the ratified *verb* row's plural); if the sentence means the layer, the ratified **action layer**. The one occurrence is Marc's ruled ch4-preamble clause ("it adopts its action set") — flag, his word. Raised by the ch4-preamble pass-6 black box (2026-09-04) | (a) ratify *action set* as the collective noun for the six verbs; (b) *action layer* (the trichotomy term); (c) *the six verbs* | tex 2026-09-04 prose: *action set* 1 (ch4 preamble), *action layer* 11 |
| The sub-question identifiers | **SQ1 / SQ2 / SQ3** as the one way to name the decomposition of the research question in prose (SQ1 capture, SQ2 model, SQ3 evaluate), always via `\ref{sq:capture}` / `\ref{sq:model}` / `\ref{sq:evaluate}` so the identifier is mechanical; the definition site is the ch1 list. Never *the first sub-question*, never *RQ1.1* (implies several top-level RQs; the single-RQ architecture (a) stands), never the bare thread names *capture / model / evaluate* as identifiers in thesis prose (they stay the writing guide's thread names). Per literature_conventions.md (g)1 (ratified 2026-08-20) and Marc's 2026-09-04 ch4 ruling; the typeset form is ratify-on-read | (a) as recommended; (b) SQ1--SQ3 plus the thread name in apposition at first use per chapter | tex 2026-09-04: SQ identifiers 6 (ch1 list ×3, ch4 preamble ×6 via \ref), *sub-question(s)* 3 (ch1 lead-in, ch4 comment trail) |

| The four evaluation methods as an ordered set | Never *ladder* / *rung* in prose for Cho's four categories: *rung* is the fidelity descriptor's word (§3.3.3 "the parametric rung", G2 "the scripted rung") and *ladder* is ch4's L0--L4 figure --- a third ladder would collide. Say *categories* or *methods*. Raised by the §3.2.3 cue card (2026-09-07); the unit as dictated uses neither word, so this is a guard, not a fix | (a) as recommended; (b) reserve *ladder* for the evaluation methods and rename the descriptor's *rung* | tex 2026-09-07 prose: *rung* 2 (both the descriptor's), *ladder* 0 in prose (comments only) |
