...
``mtdsim.run`` tells you *who won*. This tells you *how it went*: every event, in
simulated-time order, from all three actors — the attacker working through the
...
Usage
-----
    python -m mtdsim.trace                          # random scheme, 2000 t/u
    python -m mtdsim.trace --scheme none            # attacker alone: how fast unopposed?
    python -m mtdsim.trace --scheme single --mtd OSDiversity
    python -m mtdsim.trace --scheme simultaneous --finish-time 5000
    python -m mtdsim.trace --only attacker,compromise    # filter the stream
    python -m mtdsim.trace --no-colour > run.log         # plain text for a file
...
identical to the same seed run under ``mtdsim.run``.
"""
...
def run_trace(*, scheme: str = "random", mtd_interval: int | None = None,
    ...
    if scheme and scheme.lower() != "none":
        ...
        # back to the lineage's canonical experiment interval otherwise (as run.py does).
...
def main(argv=None) -> int:
    ...
    if args.mtds:
        ...
        for name in args.mtds:
            mod = importlib.import_module(f"mtdsim.mtd.{name.lower()}")
